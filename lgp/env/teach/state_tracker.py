...
class PoseInfo():
    ...
    def simulate_successful_action(self, action: TeachAction):
        #TODO This needs to be edited to match TEACH actions
        MOVE_STEP = 0.25
        PITCH_STEP = 15
        YAW_STEP = 90
...
class StateTracker():
    """
    Converts raw RGB images and executed actions to TeachObservation instances that eval:
    - Segmentation
    - Depth
    - Pose
    ...
    def __init__(self,
        ...
        if self.reference_seg:
            ...
            self.seg_model = TeachSegmentationAndDepthModel(hparams).to(PERCEPTION_DEVICE)
            self.seg_model.load_state_dict(torch.load(lgp.paths.get_segmentation_model_path()))
            self.seg_model.eval()
        if self.reference_depth:
            self.depth_model = None
        else:
            self.depth_model = TeachSegmentationAndDepthModel(hparams).to(PERCEPTION_DEVICE)
    ...
    def log_extra_events(self, events):
        self.latest_extra_events.append(events)
        self.latest_observation = self._make_observation()

    def get_observation(self) -> TeachObservation:
        return self.latest_observation

    def _make_observation(self) -> TeachObservation:
        ...
        #TODO NOTE We have to edit the pose to support this, use reference pose for now
