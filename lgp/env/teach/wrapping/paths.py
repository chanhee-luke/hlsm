...
def get_task_traj_data_path(data_split: str, task_id: str) -> str:
    traj_data_path = os.path.join(os.environ['ET_DATA'], "edh_instances", data_split, f"{task_id}.json")
    return traj_data_path

def get_game_dir_path(data_split: str):
    return os.path.join(os.environ['ET_DATA'], "games", data_split)
...
def get_splits_path():
    ...
    for split in splits_path:
        ...
        for edh_instance_name in instances_splits_files:
            data = {'task': Path(edh_instance_name).stem}
