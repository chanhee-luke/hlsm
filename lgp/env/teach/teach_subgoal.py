# TODO: This entire file is almost a duplicate of env.teach.teach_action.py. Represent the common stuff accordingly!
...
from lgp.env.teach.segmentation_definitions import OBJECT_INT_TO_STR
...
class TeachSubgoal(Subgoal, Task):
    class MakeActionException(Exception):
        ...

    """
    This is similar to TeachAction, but the argument_mask is an allocentric voxel grid
    """
    def __init__(self,
    ...
    def to(self, device):
        return TeachSubgoal(self.action_type.to(device),
    ...
    @classmethod
    def collate(cls, lst : List["TeachSubgoal"]) -> "TeachSubgoal":
    ...
    def disperse(self) -> List["TeachSubgoal"]:
        out = []
        for i in range(len(self.action_type)):
            out.append(TeachSubgoal(
    ...
    def from_type_str_and_arg_vector(cls, type_str, arg_vec):
        ...
        return TeachSubgoal(type_vec, arg_vec)

    @classmethod
    def from_type_str_arg_id_with_mask(cls, type_str, arg_id, mask):
        a = TeachSubgoal.from_type_str_and_arg_id(type_str, arg_id)
    ...
    def extract_touch_argument(cls, action, observation, event, obj_id):
        ...
        masked_semantics = argmask[None, :, :].to(semantic_image.device) * semantic_image
    ...
    def from_action_and_observation(cls, action: TeachAction, observation: TeachObservation, event, obj_id):
        ...
        if argument_mask_2d is not None:
            ...
            argument_mask_3d: VoxelGrid = ImageToVoxels()(
                argmask[None, None, :, :].float().to(depth_image.device), depth_image, extrinsics4f,
                hfov_deg)
        else:
    ...
    def to_action(self, state_repr, observation: TeachObservation, return_intermediates=False) -> TeachAction:
        ...
        device = observation.semantic_image.device
        local_object_vector = self.object_vector()
        local_object_vector = local_object_vector.to(device)
        fpv_semantic_match_mask = torch.einsum("bc,bchw->bhw", local_object_vector, observation.semantic_image.float())
        ...
        # Teach actions use 2D numpy masks
        ...
        if return_intermediates:
            return TeachAction(action_type_str, fpv_argument_mask), intermediates
        else:
            return TeachAction(action_type_str, fpv_argument_mask)

    # TODO: Check usages of this
    def get_argmax_spatial_arg_pos_xyz_vx(self):#, state_repr : Union["TeachSpatialStateRepr", None]=None):
    ...
    def __eq__(self, other: "TeachSubgoal"):
        # TODO: Consider if we need to add mask here as well
        if not isinstance(other, TeachSubgoal):
            return False
